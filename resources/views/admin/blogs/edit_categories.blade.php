    ...
    <section class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
            <nav class="flex items-center gap-2 text-on-surface-variant font-label-caps text-label-caps mb-2">
                <a href="{{ route('blogs.index') }}" class="hover:text-primary transition-colors uppercase">BLOG HUB</a>
                <span class="material-symbols-outlined text-[12px]">chevron_right</span>
                <span class="text-primary font-bold uppercase">EDIT CATEGORY</span>
            </nav>
            <h3 class="text-2xl md:text-3xl font-extrabold text-primary tracking-tight">Modify Category</h3>
            <p class="text-on-surface-variant font-body-md text-sm mt-1">Ubah atau sesuaikan nama klasifikasi kategori materi artikel.</p>
        </div>
    </section>

    <div class="bg-white rounded-2xl luxury-shadow border border-outline-variant/10 overflow-hidden max-w-2xl">
        <form action="{{ route('blog-categories.update', $category->id) }}" method="POST" class="p-8 space-y-6">
            @csrf @method('PUT')
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label class="block text-xs font-bold text-primary uppercase tracking-wider mb-2">Nama Kategori *</label>
                    <input type="text" name="name" required value="{{ old('name', $category->name) }}" id="cat_name"
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="cat_slug" readonly value="{{ old('slug', $category->slug) }}"
            ...
            <div class="border-t border-outline-variant/10 pt-4 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 bg-primary text-white rounded-xl font-title-sm text-xs font-bold flex items-center gap-2 hover:bg-primary/90 transition-all active:scale-95 duration-200 shadow-md">
                    <span class="material-symbols-outlined text-sm">published_with_changes</span><span>Update Category</span>
                </button>
            </div>
        </form>
    </div>
